		...
		{#if phase === 'shown'}
			...
			<div class="btns">
				<div>
					...
					<button class="primary" on:click={() => onClick('remove')}>はい</button>
				</div>
			</div>
		{:else}
			<p>{phase === 'success' ? 'チェックを外しました' : 'エラーが発生しました'}</p>
			<button on:click={() => onClick(phase)}>{phase === 'success' ? 'OK' : 'もう一度試す'}</button>
		{/if}
	</div>
</div>
